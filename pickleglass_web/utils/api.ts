...
export const getUserInfo = (): UserProfile | null => {
  ...
  const storedUserInfo = localStorage.getItem('thoughts_user');
  if (storedUserInfo) {
    try {
      return JSON.parse(storedUserInfo);
    } catch (error) {
      console.error('Failed to parse user info:', error);
      localStorage.removeItem('thoughts_user');
    }
  }
  return null;
};
...
export const setUserInfo = (userInfo: UserProfile | null, skipEvents: boolean = false) => {
  ...
  if (userInfo) {
    localStorage.setItem('thoughts_user', JSON.stringify(userInfo));
  } else {
    localStorage.removeItem('thoughts_user');
  }
